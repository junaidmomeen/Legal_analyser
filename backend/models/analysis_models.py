...
class KeyClause(BaseModel):
    ...
    type: str = Field(..., description="Type of clause (e.g., 'Payment Terms', 'Termination')")
    content: str = Field(..., description="Full text content of the clause")
    importance: Literal['high', 'medium', 'low'] = Field(..., description="Importance level of the clause")
    classification: Literal['Contractual', 'Compliance', 'Financial', 'Termination', 'Confidentiality', 'Miscellaneous'] = Field(..., description="Classification of the clause")
    risk_score: float = Field(default=0.0, ge=0.0, le=10.0, description="Risk score for the clause (1-10)")
    page: Optional[int] = Field(None, description="Page number where clause was found")
    confidence: float = Field(default=0.0, ge=0.0, le=1.0, description="Confidence score for clause extraction")

class AnalysisResult(BaseModel):
    """Model for the complete document analysis result"""
    summary: str = Field(..., description="AI-generated summary of the document")
    key_clauses: List[KeyClause] = Field(default_factory=list, description="List of extracted key clauses")
    ...
    processing_time: float = Field(default=0.0, description="Time taken to process the document in seconds")
    word_count: int = Field(default=0, ge=0, description="Total word count in the document")
...
class DocumentProcessingResult(BaseModel):
    ...
    extracted_text: str = Field(default="", description="Extracted text from the document")
