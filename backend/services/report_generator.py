...
class ReportGenerator:
    ...
    def export_as_json(self, analysis: Dict[str, Any], original_filename: str) -> str:
        ...
        try:
            ...
            file_path = f"exports/{self._sanitize_filename(original_filename)}_analysis.json"
    ...
    def _build_header_section(self, analysis: Dict[str, Any], original_filename: str) -> List[Any]:
        ...
        # Main title
        elements.append(Paragraph("Legal Document Analysis Report", self.styles['ReportTitle']))
    ...
    def _build_executive_summary(self, analysis: Dict[str, Any]) -> List[Any]:
        ...
        for i, sentence in enumerate(sentences):
            para += sentence + ' '
            if (i + 1) % 4 == 0:
                elements.append(Paragraph(self._bold_keywords(para), self.styles['ExecutiveSummary']))
                para = ''
        if para:
            elements.append(Paragraph(self._bold_keywords(para), self.styles['ExecutiveSummary']))
    ...
    def _build_visual_insights_section(self, analysis: Dict[str, Any]) -> List[Any]:
        ...
        if len(clauses) > 0:
            ...
            if len(high_importance_clauses) / len(clauses) > 0.5:
                elements.append(Paragraph("⚠ HIGH RISK DOCUMENT", self.styles['RiskMarker']))
        ...
        if len(clauses) > 2:  # Only create charts if we have sufficient data
            ...
            if avg_risk_score is not None:
                elements.append(Paragraph("Average Risk Score", self.styles['SubHeader']))
                elements.append(self._create_gauge_chart(avg_risk_score, 1, 10, width=400, height=40))
                elements.append(Spacer(1, 15))

            # Importance Distribution Pie Chart
            importance_chart = self._create_importance_pie_chart(clauses)
            if importance_chart:
                elements.append(Paragraph("Clause Importance Distribution", self.styles['SubHeader']))
            ...
            if len(clauses) > 4:  # Only for more complex documents
                ...
                if type_chart:
                    elements.append(Paragraph("Clause Types Distribution", self.styles['SubHeader']))
                    elements.append(type_chart)
                    elements.append(Spacer(1, 20))
        else:
            elements.append(Paragraph("Insufficient data for visual analysis.", self.styles['ClauseContent']))
            elements.append(Spacer(1, 20))
    ...
    def _build_key_clauses_section(self, analysis: Dict[str, Any]) -> List[Any]:
        ...
        clauses = analysis.get('key_clauses', [])
        if not clauses:
            elements.append(Paragraph("No key clauses identified.", self.styles['ClauseContent']))
            return elements
        
        # Sort clauses by importance and type
        sorted_clauses = sorted(clauses, key=lambda x: (
            {'high': 0, 'medium': 1, 'low': 2}.get(x.get('importance', 'low').lower(), 2),
            x.get('type', 'Unknown')
        ))
        ...
        for i, clause in enumerate(sorted_clauses, 1):
            ...
            page_info = f" (Page {clause.get('page', 'N/A')})" if clause.get('page') else ""
            ...
            badges_data = [
                [f"Importance: {importance.capitalize()}", f"Risk: {clause.get('risk_level', 'N/A')}"]
            ]
            
            badges_table = Table(badges_data, colWidths=[2.5*inch, 2.5*inch])
            badges_table.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (0, 0), self.risk_colors.get(importance, self.colors['neutral'])),
                ('BACKGROUND', (1, 0), (1, 0), self.risk_colors.get(clause.get('risk_level', 'low').lower(), self.colors['neutral'])) , 
            ...
            if len(content) > 500:
                content = content[:500] + "..."
            clause_elements.append(Paragraph(self._bold_keywords(f"<b>Content:</b> {content}"), self.styles['ClauseContent']))
            
            # Explanation if available
            explanation = clause.get('explanation', '')
            if explanation:
                clause_elements.append(Paragraph(self._bold_keywords(f"<b>Analysis:</b> {explanation}"), self.styles['ClauseContent']))
    ...
    def _build_risk_assessment_section(self, analysis: Dict[str, Any]) -> List[Any]:
        ...
        if recommendations:
            elements.append(Paragraph(f"• {recommendations[0]}", self.styles['ClauseContent']))
    ...
    def _create_importance_pie_chart(self, clauses: List[Dict[str, Any]]) -> Optional[Drawing]:
        ...
        try:
            ...
            importance_counts = Counter(clause.get('importance', 'low').lower() for clause in clauses)
            ...
            for importance in ['high', 'medium', 'low']:
                ...
                if count > 0:
                    ...
                    colors_list.append(self.risk_colors.get(importance, self.colors['neutral']))
    ...
    def _create_clause_type_bar_chart(self, clauses: List[Dict[str, Any]]) -> Optional[Drawing]:
        ...
        try:
            ...
            labels = [type_name[:15] + "..." if len(type_name) > 15 else type_name 
    ...
    def _generate_statistics(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        risk_counts = Counter(clause.get('risk_level', 'low').lower() for clause in clauses)
    ...
    def _generate_key_insights(self, analysis: Dict[str, Any]) -> List[str]:
        ...
        if high_importance > 0:
            insights.append(f"Found {high_importance} high-importance clause{'s' if high_importance != 1 else ''} requiring attention")
        
        # Risk insights
        high_risk = stats['risk_counts'].get('high', 0)
        if high_risk > 0:
            insights.append(f"Identified {high_risk} high-risk clause{'s' if high_risk != 1 else ''} that may need legal review")
        ...
        if stats['unique_types'] > 5:
            insights.append("Document contains diverse clause types indicating comprehensive coverage")
        
        # Confidence insights
        confidence = analysis.get('confidence', 0)
        if confidence < 0.7:
            insights.append("Analysis confidence is moderate - manual review recommended")
        elif confidence > 0.9:
