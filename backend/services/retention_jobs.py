...
class RetentionJobManager:
    ...
    async def _cleanup_analysis_cache(self):
        ...
        cutoff_time = datetime.now() - timedelta(hours=self.config.analysis_retention_hours)
        cleanup_count = 0
    ...
    async def _cleanup_export_files(self):
        ...
        cutoff_time = datetime.now() - timedelta(hours=self.config.export_file_retention_hours)
        cleanup_count = 0
    ...
    async def _health_check(self):
        ...
        try:
            ...
            # Log periodic status
            logger.debug(f"Retention health check - Temp: {temp_usage/1024:.1f}KB, "
