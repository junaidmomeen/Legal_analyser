...
async def deep_health_check(request: Request):
    ...
    try:
        ...
        health_checks["checks"]["disk_space"] = {
            "status": "healthy" if free_space_percent > 10 else "warning" if free_space_percent > 5 else "critical",
    ...
    try:
        ...
        open_circuits = [name for name, state in circuit_breakers.items() if state["state"] == "open"]
        if open_circuits:
