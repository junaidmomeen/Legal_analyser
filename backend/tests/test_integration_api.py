...
def test_analyze_success(test_client):
    ...
    with patch("utils.file_validator.magic.from_buffer", return_value="application/pdf"):
        ...
        ai_analyzer.analyze_document = AsyncMock(return_value=AnalysisResult(
        ...
        resp = test_client.post("/analyze", files=files, headers={"Authorization": f"Bearer {token}"})
    ...
    export = test_client.post(f"/export/{file_id}/json", headers={"Authorization": f"Bearer {token}"})
    assert export.status_code == 200
    task_id = export.json()["task_id"]

    status_resp = test_client.get(f"/export/{task_id}", headers={"Authorization": f"Bearer {token}"})
    assert status_resp.status_code == 200
    if status_resp.json().get("status") == "ready":
        dl_token = status_resp.json()["download_token"]
        dl_resp = test_client.get(f"/export/{task_id}/download", params={"token": dl_token})
        assert dl_resp.status_code in (200, 400)  # 200 if finished in time, 400 if still processing
...
def test_rate_limiting_trigger(test_client):
    ...
    with patch("utils.file_validator.magic.from_buffer", return_value="application/pdf"):
        ...
        for _ in range(12):
            r = test_client.post("/analyze", files=files, headers={"Authorization": f"Bearer {token}"})
    ...
    assert any(
        test_client.post(
            "/analyze",
            files={"file": ("t.pdf", io.BytesIO(make_dummy_pdf_bytes()), "application/pdf")},
