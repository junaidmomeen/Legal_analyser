...
async def test_process_pdf(document_processor):
    ...
    with patch('fitz.open', return_value=mock_doc):
        with patch('pytesseract.image_to_string', return_value='This is a test PDF file.'):
            with patch('os.path.exists', return_value=True):
                with patch('os.path.getsize', return_value=1000):
                    result = await document_processor.process_document('dummy.pdf', file_type='pdf')
...
async def test_process_image(document_processor):
    ...
    with patch('PIL.Image.open', return_value=mock_image):
        with patch('pytesseract.image_to_string', return_value='This is a test image file.'):
            with patch('os.path.exists', return_value=True):
                with patch('os.path.getsize', return_value=500):
                    result = await document_processor.process_document('dummy.png', file_type='image')
                    assert result.success
